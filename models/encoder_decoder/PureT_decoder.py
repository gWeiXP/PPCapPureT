...
class Decoder(nn.Module):
    ...
    def forward(self, gx, seq, encoder_out, seq_mask=None, att_mask=None):
        ...
        seq_len = seq.size()[1]
        pos_indx = torch.arange(1, seq_len + 1).view(1, -1).to(seq.device)
        if self.seq_len is not None:
            seq_len = self.seq_len + seq_len
            self.seq_len = seq_len
            pos_indx = torch.arange(seq_len, seq_len + 1).view(1, -1).to(seq.device)
    ...
    def forward(self, gx, x, encoder_out, seq_mask, att_mask=None):
        ...
        if self.use_gx:
            ...
            if att_mask is not None:
                ...
                _att_mask = torch.cat(
                    [att_mask, torch.ones(att_mask.size(0)).unsqueeze(1).unsqueeze(1).to(att_mask.device)], 2
                ).long()
            else:
                _att_mask = None
        else:
...
class MultiHeadSelfAttention(nn.Module):
    ...
    def init_buffer(self, batch_size):
        # [B, nH, 0, C/nH]
        self.buffer_key = torch.zeros((batch_size, self.num_heads, 0, self.head_dim), device=next(self.parameters()).device)
        self.buffer_value = torch.zeros((batch_size, self.num_heads, 0, self.head_dim), device=next(self.parameters()).device)
