...
class PreTrainedModel(nn.Module, ModuleUtilsMixin):
    ...
    def _generate_no_beam_search(
        ...
        if not(gedi_model is None):
            ...
            # base模型
            prefix_sequence = base_model.clip_project(prefix_sequence).view(-1, base_model.prefix_length, base_model.gpt_embedding_size)
        ...
        while cur_len < max_length:
            if past is not None:
                ...
                model_inputs = {"inputs_embeds": prefix_sequence, "past": past}
            ...
            if not(gpt3_api_key is None):
                ...
                # # lookup
                # values_base, indices_base = torch.topk(torch.softmax(next_token_logits,-1), 10, dim=1)
                # lookup_base = showshowshow(input_ids, values_base, indices_base, tokenizer)
                # lookup_base2 = showshowshow2(torch.softmax(next_token_logits,-1))
            ...
            if not(gedi_model is None):
                ...
                # # lookup
                # values_a, indices_a = torch.topk(torch.exp(gedi_outputs[0][:(gedi_outputs[0].shape[0]//2), -1, :]), 10, dim=1)
                # lookup_a = showshowshow(input_ids, values_a, indices_a, tokenizer)
                # lookup_a2 = showshowshow2(torch.exp(gedi_outputs[0][:(gedi_outputs[0].shape[0]//2), -1, :]))
                # values_b, indices_b = torch.topk(torch.exp(gedi_outputs[0][(gedi_outputs[0].shape[0]//2):, -1, :]), 10, dim=1)
                # lookup_b = showshowshow(input_ids, values_b, indices_b, tokenizer)
                # lookup_b2 = showshowshow2(torch.exp(gedi_outputs[0][(gedi_outputs[0].shape[0]//2):, -1, :]))

                # # lookup
                # values_a, indices_a = torch.topk(torch.softmax(gedi_outputs[0][:(gedi_outputs[0].shape[0]//2), -1, :], -1), 10, dim=1)
                # lookup_a = showshowshow(input_ids, values_a, indices_a, tokenizer)
                # lookup_a2 = showshowshow2(torch.softmax(gedi_outputs[0][:(gedi_outputs[0].shape[0]//2), -1, :], -1))
                # values_b, indices_b = torch.topk(torch.softmax(gedi_outputs[0][(gedi_outputs[0].shape[0]//2):, -1, :], -1), 10, dim=1)
                # lookup_b = showshowshow(input_ids, values_b, indices_b, tokenizer)
                # lookup_b2 = showshowshow2(torch.softmax(gedi_outputs[0][(gedi_outputs[0].shape[0]//2):, -1, :], -1))

                if gedi_past is None:
                    if gedi_outputs[0].shape[1]>seq_batched_embedding.shape[1]:
                ...
                seq_len= seq_len+1
                # gedi_logits= (gedi_outputs[0][:, -1, :]+logits_r.unsqueeze(1))
                ...
                # logp_desired_softmax = torch.exp(logp_desired_t)
                # logp_desired_softmax_mask = (logp_desired_softmax >= 0.25) & (logp_desired_softmax <= 0.75)
                # logp_desired_softmax[~logp_desired_softmax_mask] = logp_desired_softmax[~logp_desired_softmax_mask]+0.5
                # logp_desired_softmax[logp_desired_softmax_mask] = logp_desired_softmax[logp_desired_softmax_mask]/(1.0-logp_desired_softmax)[logp_desired_softmax_mask]/2.4
                # logp_desired_t = torch.log(logp_desired_softmax)


                # # lookup
                # values_gedi_d, indices_gedi_d = torch.topk(torch.exp(logp_desired_t), 10, dim=1)
                # lookup_d = showshowshow(input_ids, values_gedi_d, indices_gedi_d, tokenizer)
                # lookup_d2 = showshowshow2(torch.exp(logp_desired_t))

                # # lookup
                # values_gedi, indices_gedi = torch.topk(torch.exp(logp_desired_t-torch.log(1-torch.exp(logp_desired_t))), 10, dim=1)
                # lookup_gedi = showshowshow(input_ids, values_gedi, indices_gedi, tokenizer)
                # lookup_gedi2 = showshowshow2(torch.exp(logp_desired_t-torch.log(1-torch.exp(logp_desired_t))))


                # 第一个单词由base决定，从第二个单词开始
                if cur_len <= 1:
                    ...
                    # next_token_logits = torch.log_softmax(gedi_outputs[0][:(gedi_outputs[0].shape[0]//2), -1, :],-1)
                    # next_token_logits = torch.log_softmax(1*next_token_logits,-1) + disc_weight * (torch.log_softmax(gedi_outputs[0][:(gedi_outputs[0].shape[0]//2), -1, :],-1))#貌似是对比的，强硬结合
                # next_token_logits = torch.log_softmax(1*next_token_logits,-1) + disc_weight*(logp_desired_t-logp_undesired_t)

                # test1 = torch.exp(logp_desired_t)
                # test2 = torch.exp(logp_undesired_t)
                # test3 = torch.exp(torch.log(1-torch.exp(logp_desired_t)))
                # test4 = torch.exp(logp_desired_t-logp_undesired_t)
                # test5 = torch.exp(logp_desired_t-torch.log(1-torch.exp(logp_desired_t)))

                

                # next_token_logits = torch.log_softmax(gedi_outputs[0][:(gedi_outputs[0].shape[0]//2), -1, :],-1)
                
                # next_token_logits = torch.log_softmax(torch.split(gedi_outputs[0][:, -1, :], input_ids.shape[0])[0], -1)
                # next_token_logits = torch.log_softmax(1*next_token_logits,-1) + disc_weight*(logp_desired_t) #+delta_capped82058721
                # next_token_logits = torch.log_softmax(1*next_token_logits,-1) + disc_weight * torch.log_softmax(torch.split(gedi_outputs[0][:, -1, :], input_ids.shape[0])[0], -1)

                


                # # # lookup
                # values_generate, indices_generate = torch.topk(torch.exp(next_token_logits), 10, dim=1)
                # lookup_generate = showshowshow(input_ids, values_generate, indices_generate, tokenizer)
                # lookup_generate2 = showshowshow2(torch.exp(next_token_logits))

                # values_generate, indices_generate = torch.topk(torch.softmax(next_token_logits, -1), 10, dim=1)
                # lookup_generate = showshowshow(input_ids, values_generate, indices_generate, tokenizer)
                # lookup_generate2 = showshowshow2(torch.softmax(next_token_logits, -1))
